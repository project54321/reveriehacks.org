...
const BentoCardGrid: React.FC<{ children: React.ReactNode; gridRef?: React.RefObject<HTMLDivElement | null>; }> = ({ children, gridRef }) => (
  <div className="bento-section grid gap-2 p-3 max-w-[72rem] select-none relative" style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
    {children}
  </div>
);
...
const useMobileDetection = () => {
  ...
  useEffect(() => { const checkMobile = () => setIsMobile(window.innerWidth <= MOBILE_BREAKPOINT); checkMobile(); window.addEventListener('resize', checkMobile); return () => window.removeEventListener('resize', checkMobile); }, []);
  return isMobile;
};

const MagicBento: React.FC<BentoProps> = ({ textAutoHide = true, enableStars = true, enableSpotlight = true, enableBorderGlow = true, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, particleCount = DEFAULT_PARTICLE_COUNT, enableTilt = false, glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = true, cards }) => {
  const gridRef = useRef<HTMLDivElement>(null);
  const isMobile = useMobileDetection();
  const shouldDisableAnimations = disableAnimations || isMobile;
  const theme = useTheme();
  const cardData = cards && cards.length ? cards : defaultCardData;
  // Theme-aware card background: in light mode use var(--card) with subtle purple border
      ...
      <style>
        {`
          .bento-section {
            ...
            --border-color: ${isLight ? 'var(--border)' : '#2F293A'};
            --background-dark: ${isLight ? 'var(--card)' : '#120F17'};
            --white: ${isLight ? 'var(--foreground)' : 'hsl(0, 0%, 100%)'};
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            ...
            const cardStyle = {
              backgroundColor: isLight ? 'var(--card)' : (card.color || 'var(--background-dark)'),
              borderColor: isLight ? 'var(--border)' : 'var(--border-color)',
              color: isLight ? 'var(--foreground)' : 'var(--white)',
              '--glow-x': '50%',
              '--glow-y': '50%',
              '--glow-intensity': '0',
              '--glow-radius': '200px'
            ...
            if (enableStars) {
              return (
                <ParticleCard key={index} className={baseClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                  <div className="card__header flex justify-between gap-3 relative" style={{ color: isLight ? 'var(--foreground)' : 'white' }}>
                    <span className="card__label text-base">{card.label}</span>
                  </div>
                  <div className="card__content flex flex-col relative" style={{ color: isLight ? 'var(--foreground)' : 'white' }}>
            ...
            return (
              <div key={index} className={baseClassName} style={cardStyle} ref={el => {
                if (!el) return;
                const handleMouseMove = (e: MouseEvent) => {
                  ...
                  if (enableMagnetism) {
                    const magnetX = (x - centerX) * 0.05; const magnetY = (y - centerY) * 0.05;
