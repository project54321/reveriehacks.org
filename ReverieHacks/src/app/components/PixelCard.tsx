...
interface PixelCardProps { variant?: 'default' | 'blue' | 'yellow' | 'pink'; gap?: number; speed?: number; colors?: string; noFocus?: boolean; className?: string; children: React.ReactNode; }
interface VariantConfig { activeColor: string | null; gap: number; speed: number; colors: string; noFocus: boolean; }
export default function PixelCard({ variant = 'default', gap, speed, colors, noFocus, className = '', children }: PixelCardProps): JSX.Element {
  ...
  const lightColors = variant === 'pink' ? '#f3e8ff,#e9d5ff,#8b5cf6' : '#f5f3ff,#ede9fe,#8b5cf6';
  const effectiveColors = theme === 'light' && !colors ? lightColors : finalColors;
  ...
  const initPixels = () => {
    ...
    const colorsArray = effectiveColors.split(',');
    const pxs = [];
  ...
  const onFocus: React.FocusEventHandler<HTMLDivElement> = e => { if (e.currentTarget.contains(e.relatedTarget as Node)) return; handleAnimation('appear'); };
  const onBlur: React.FocusEventHandler<HTMLDivElement> = e => { if (e.currentTarget.contains(e.relatedTarget as Node)) return; handleAnimation('disappear'); };
  useEffect(() => {
    initPixels();
  ...
  }, [finalGap, finalSpeed, effectiveColors, finalNoFocus, theme]);
  return (
    <div ref={containerRef} className={`relative overflow-hidden grid place-items-center rounded-[20px] border isolate transition-colors duration-200 select-none ${theme === 'light' ? 'border-border bg-card' : 'border-[#27272a] bg-card'} ${className}`} style={{ borderColor: 'var(--border)', background: 'var(--card)' }} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onFocus={finalNoFocus ? undefined : onFocus} onBlur={finalNoFocus ? undefined : onBlur} tabIndex={finalNoFocus ? -1 : 0}>
      <canvas className="w-full h-full block" ref={canvasRef} />
      <div className="absolute inset-0 grid place-items-center pointer-events-none [&>*]:pointer-events-auto">
        {children}
      </div>
    </div>
  );
